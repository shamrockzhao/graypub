---
-- @classmod grayPub.adapter.policy
-- @release 0.0.1
local modulename = "grayPubAdapterPolicy"
...
_M.set = function(self, policy)
    ...
    local database = self.database
    local divModulename = table.concat({'grayPub', 'diversion', policy.divtype}, '.')
...
_M.get = function(self, id)
    ...
    local divModulename = table.concat({'grayPub', 'diversion', divtype}, '.')
